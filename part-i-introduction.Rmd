...
Fixing the intercept at the estimate $\beta_{0} = 7.4225$, it looks like this. The point shows the frequentist linear regression estimate, which is at the very bottom.
...
The prior, $p(\beta)$, quantifies our prior knowledge about the effect.

Might come from, previous studies, common sense, physical knowledge, etc.
...
A quite common choice would be a uniform prior over some large range, e.g., $[-1000, 1000]$, or even $[-\infty, \infty]$. This means that all values in the range are considered equally likely a priori.



```{r, echo=FALSE, fig.height=4, fig.width=6}
tibble(beta = seq(from = -1100, to = 1100, by = 1)) %>% 
...
- Convenient, but often very unrealistic. In this case $\beta$ represents the effect of vitamin C dose on growth of teeth, and $\pm 1000$ are probably impossible for a large number of reasons.


---

- Assume we consider the biology of the problem, and find that values of $\beta$ beyond $\pm 50$ are extremely unlikely. We can encode this with a normal prior, with standard deviation 25, so there is a 95 % probability that $\beta$ is in $[-50, 50]$. To avoid biasing the effect in a certain direction, we set the mean to zero.
